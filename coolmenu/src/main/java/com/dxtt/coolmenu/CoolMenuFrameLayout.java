...
//    private static final String TAG = CoolMenuFrameLayout.class.getName();
    ...
    private void initAnim() {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            TranslateLayout child = (TranslateLayout) getChildAt(i);

            float transX = (float) ((i + 1) * 0.06);
            float transY = (float) ((i + 1) * 0.08); //old value is 0.12
            ...
            //animator = ObjectAnimator.ofFloat(child, "XFraction", "YFraction", valuesHolderX, valuesHolderY);
    ...
    @UiThread
    public void setMenuTitleSize(@NonNull float size) {
        for (int i = 0; i < num; i++) {
            ((TranslateLayout) getChildAt(i)).setMenuTitleSize(size);
        }
    }

    @UiThread
    public void setMenuTitleColor(@NonNull List<Integer> colorResIds) {
        for (int i = 0; i < num; i++) {
            ((TranslateLayout) getChildAt(i)).setMenuTitleColor(colorResIds.get(i));
        }
    }
